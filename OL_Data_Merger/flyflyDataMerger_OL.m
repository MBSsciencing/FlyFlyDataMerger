...
% FLYFLYDATAMERGER M-file for flyflyDataMerger_OL.fig
...
% Last Modified by Yuri 7 Dec 2023
% This DataMerger for Open loop behavioral data
% Changes made line in 415 to save all values stored in data_block
...
%         dataBlock = eval(['data_block' num2str(n) '(2,:)']); %REVERSED 
        %%It was like above, but to save all values in data_block, Changed to 
        dataBlock = eval(['data_block' num2str(n) '(1:end,:)']); %REVERSED
%         dataBlock = dataBlock(~isnan(dataBlock)); %remove NaN % Do not do
%         this
